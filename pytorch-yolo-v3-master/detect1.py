...
def arg_parse():
    ...
    "Image / Directory containing images to perform detection upon",
                        default="result_1_2.jpg", type=str)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for i, v in enumerate(chair):
            ...
            # cv2.line(img,(0,500),(3000,500),[0,255,255],20)
        ...
        #     cur.execute(sql, (i,str(int(Left_x1)), str(int(check_y)),
        ...
        if len(person) == 0:
            update_sql = "update posit set check_=0,count=0 where seatnumber <=3"
            ...
            for j, re in enumerate(result):
                for i, v in enumerate(person):
                    ...
                    if (int(re[1]) <= int(center_x) and int(center_x) <= int(re[3])):
                        ...
                        if int(Right_y2) >= int(re[4]) and int(Left_y1) >= 800:
                            # and int(Left_y1) >= 500:
                        ...
                        #     break
                            ...
                            # 의자의 x축안에 사람의 중점좌표가 있지만 y축을 벗어난경우 하단 y축은 맞음
        ...
        cv2.imwrite(os.path.join("", "det/result_1_1.jpg"), img)

        end = time.time()
        sql = "update wait set image3=0"
        cur.execute(sql)
        db.commit()
