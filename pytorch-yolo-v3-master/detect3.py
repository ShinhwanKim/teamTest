...
if __name__ == '__main__':
    ...
    while True:
        ...
        for i, v in enumerate(chair):
            ...
            cv2.line(img,(0,1000),(3000,1000),[0,255,255],20)
        ...
        if len(person) == 0:
            update_sql = "update posit set check_=0,count=0 where seatnumber <=11 and seatnumber >7"
            cur.execute(update_sql)
        else:

            sql = "select * from posit where seatnumber <=11 and seatnumber >7"
            cur.execute(sql)
            result = cur.fetchall()
            for j, re in enumerate(result):
                for i, v in enumerate(person):
                    ...
                    # cv2.line(img, (0, 1200), (3000, 1200), [0, 255, 255], 20)
                    # cv2.line(img, (0, 700), (3000, 700), [0, 255, 255], 20)
                    # cv2.line(img, (0, 1100), (3000, 1100), [0, 255, 255], 20)
                    ...
                    if (int(re[1]) <= int(center_x) and int(center_x) <= int(re[3])):
                        ...
                        if int(Right_y2) >= int(re[4]) and int(Left_y1) >= 1000:
                            # and int(Left_y1) >= 700:
        ...
        # cv2.imwrite(os.path.join("", "a160hand_result.jpg"), img)
