...
def arg_parse():
    ...
    "Image / Directory containing images to perform detection upon",
                        default="asdf/0index36.jpg", type=str)
    ...
    while True:
        ...
        try:
            imlist = [osp.join(osp.realpath('.'), images, img) for img in os.listdir(images) if
                      os.path.splitext(img)[1] == '.png' or os.path.splitext(img)[1] == '.jpeg' or
        ...
        def write(x, batches, results):
            ...
            cls = int(x[-1])
            y1= c1[1]
            if  str(format(classes[cls])) == 'person' and c1[1]>=900:
                ...
                if (name == "person"):
                    person.append(str(name) + "/" + str(c1[0]).replace("tensor(", "").split(",")[0] + "/" +
                ...
                # for i in chair:
                #     print(str(i)+"\n")
                # sql="select count(*) as sehoon from posit where seatnumber>7 and seatnumber <=11"
                # cur.execute(sql)
                # num=cur.fetchall()
                # if(int(num[0])!=12):
                ...
                #     "and (position1Y>=%s and position2Y<=%s) and (seatnumber>7 and seatnumber<=11)"
                #     cur.execute(sql,())
        ...
        person.sort(reverse=True, key=lambda x: float(x.split("/")[1]))
        # print(chair)
        for i, v in enumerate(chair):
            ...
            # cv2.line(img,(0,900),(3000,900),[0,255,255],20)
        ...
        #     #       "(position2Y>=%s and position2Y<=%s) and (seatnumber>7 and seatnumber<=11)"
        ...
        #     cur.execute(sql, (i+8,str(int(Left_x1)), str(int(check_y)),
        #                       str(int(Right_x2)), str(int(check_y)), 0))
        #
        sql = "update posit set check_=0 where seatnumber>7 and seatnumber<=11"
        cur.execute(sql)
        for i, v in enumerate(person):
            ...
            path = ""
            # cv2.imwrite(os.path.join(path, "1.jpg"), img)
            sql = "select * from posit where (seatnumber>7 and seatnumber<=11)"
            cur.execute(sql)
            result = cur.fetchall()
            for i, re in enumerate(result):
                print("쿼리문")
                if ((int(re[1]) <= int(center_x) and int(center_x) <= int(re[3])) and int(Right_y2) >= int(re[4])):
                    cv2.putText(img, "True", (center_x + 10, center_y + 50), cv2.FONT_HERSHEY_PLAIN, 3, [225, 255, 255],
                                3)
                    # plt.text(center_x + 7, center_y + 50, "(True)",color=str("w"), size="7")
        ...
        cv2.imwrite(os.path.join("", "190902/1125am/default1.jpg"), img)
